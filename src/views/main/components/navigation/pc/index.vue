<template>
	<div class="bg-white dark:bg-zinc-800 duration-500 sticky top-0 left-0 w-full z-10">
		...
		>
			...
			<!-- category item -->
			<li
				v-for="(item, index) in store.categorys"
				:key="item.name"
				...
				:class="{
					'text-zinc-900 dark:text-zinc-300 bg-zinc-200 dark:bg-zinc-900': store.currentCategoryIndex === index
				}"
...
<script setup lang="ts" name="navigationPc">
import { ref } from 'vue'
 ...
 * 选中状态处理
 */
const onItemClick = (item: ICategoryItem) => {
	store.useCurrentCategory(item)
}
</script>
