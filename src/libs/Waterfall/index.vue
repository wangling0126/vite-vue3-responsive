...
<script setup lang="ts" name="mWaterfall">
import { computed, nextTick, onMounted, onUnmounted, Ref, ref, watch } from 'vue'
import { getAllImg, getImgElements, getMaxHeight, getMinHeight, getMinHeightColumn, onComplateImgs } from '@/libs/Waterfall/utils'
import { IPexelsItem } from '@/store/interface/index'
const props = defineProps({
	// 数据源
	data: {
		type: Array as () => Array<IPexelsItem>,
		required: true
	},
	// 唯一标识的 key
	nodeKey: {
 ...
 * }
 * */
export interface IColumnHeightObjType {
	[key: number | string]: number
}
const columnHeightObj: Ref<IColumnHeightObjType> = ref({})
/**
 * 构建记录各列的高度的对象。
 */
...
// 容器实例
const containerTarget: Ref<HTMLInputElement | null> = ref(null)
...
const useContainerWidth = () => {
	...
	/*
	 * 容器宽度
	 * 用getComputedStyle获取的paddingLeft, paddingRight都是带单位的，所以需要parseFloat
	 * */
...
const waitImgComplate = () => {
	itemHeights = []
	// 拿到所有元素
	let itemElements: HTMLElement[] = Array.from(
		document.getElementsByClassName('m-waterfall-item') as HTMLCollectionOf<HTMLElement>
	)
	// 获取所有元素的 img 标签
...
const useItemHeight = () => {
	itemHeights = []
	// 拿到所有元素
	let itemElements = Array.from(document.getElementsByClassName('m-waterfall-item') as HTMLCollectionOf<HTMLElement>)
	// 计算 item 高度
	itemElements.forEach(el => {
		// 依据传入数据计算出的 img 高度
		itemHeights.push(el.offsetHeight)
